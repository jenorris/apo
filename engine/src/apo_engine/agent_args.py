...
def resolve_body_text(
    ...
    *,
    ...
    """Resolve note body. ``content``≡``text``; ``body`` is a legacy alias for either.

    Returns ``(body, alias_key, error_message)``. ``alias_key`` is None when the
    canonical key was used; otherwise ``content``, ``text``, or ``body``.
    """
    canonical = prefer
    provided: list[tuple[str, str]] = []
    if text is not None:
        provided.append(("text", text))
    if content is not None:
        provided.append(("content", content))
    if body is not None:
        provided.append(("body", body))
    if not provided:
        return None, None, (
            f"missing {canonical}= (aliases content=, text=, body= — prefer {canonical}=)"
        )
    values = [v for _, v in provided]
    if len(set(values)) > 1:
        keys = ", ".join(k + "=" for k, _ in provided)
        return None, None, (
            f"conflicting body args ({keys}); pass only one ({canonical}= is canonical)"
        )
    resolved = values[0]
    alias_key = None if any(k == canonical for k, _ in provided) else provided[0][0]
    return resolved, alias_key, None
