...
`search_notes` also accepts `vaults=["meta", "work"]` to fan out across those indexes, merge hits by score, and return top `limit` (each hit stamped with `vault`). Do not pass `vault=` and `vaults=` together.
...
search_notes("password manager", vaults=["meta", "work"], limit=8)
