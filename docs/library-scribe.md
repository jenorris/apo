...
**Status:** Design (2026-08-14) — not yet implemented.
...
**Inline:** successful writes attach `flaws` for that path.
**Sweep:** `vault(action=lint)` (proposed) — folder/vault backlog, paginated.
**Batch:** existing `vault-tools` OKF lint/fix share detector `code`s where possible.
...
| **B (shipped)** | Archival suggest: `flaws[]` + `vault(action=lint)` — [contracts/archival.md](contracts/archival.md) |
| 1 | Broader `flaws[]` on write; migrate soft OKF |
| 2 | General lint detectors beyond archival |
| 3 | Mechanical auto-fix + echo |
| 4 | Links / dialect / layout detectors |
| 5 | Habit KPIs (`flaws_emitted`, …) |
